...
unsigned char form_value[1024]={0};
									...
									char *ack_json, int ack_len) //Post方式上传图片
{
	...
	unsigned long devicesize = snprintf(form_value,1024,HTTP_DATA,http_boundary,DeviceID);
	...
	totalsize = filesize + devicesize + request_len + end_len;
	
	//snprintf(http_param,256,SERVER_PATH,device_info.devId);//HTTP参数设置
	snprintf(http_param,256,SERVER_PATH);//HTTP参数设置
	printf("http_param = %s\n",http_param);
    ...
    strcat(request,form_value);  									//http头信息
	...
	int readbyte = fread(request+head_len+ devicesize +request_len, 1, filesize, fp);//读取上传的图片信息
    if(readbyte < 1) //小于1024个字节 则认为图片有问题
	{
		printf("Read picture data fail!\r\n");  
        return -1;
	}
    memcpy(request+head_len+ devicesize +request_len+filesize,send_end,end_len);  //http结束信息
	...
	printf("start receive post data");
	/*********  接受http post 回复信息 ***********/
